...
void processSerialData(const uint8_t *data);
/* USER CODE BEGIN PFP */
...
volatile uint8_t uartRxBuffer[50];
   ...
   while (1)
     ...
     // Traitez les données reçues
   	  ...
   	  DHT_GetData(&DHT11_Data);
   	 	  ...
   	 	  HAL_Delay(3000);
   	 	   char receivedData[50]; // Taille du tampon de réception
   	 	  char buffer1[50];
   	 	  char buffer2[50];
   	 	     sprintf(buffer1, "Temperature: %.2f degrees Celsius  ", Temperature);
   	 	     sprintf(buffer2, " humidity: %.2f\r\n ",  Humidity);
...
/**
  ...
  * @retval None
  */
// UART Receive Callback Function (add this in your code)
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart == &huart2) {
        ...
        HAL_UART_Receive_IT(&huart2, &uartRxBuffer[uartRxIndex], 1);  // Continue reception
    }
}
void processSerialData(const uint8_t *data) {
    // Implement your data processing logic here
    // For now, just echo back the received data
    HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen((const char *)data), HAL_MAX_DELAY);
}
void SystemClock_Config(void)
{
